...
def gen_report_for_per_frame_model(per_frame_csv=None, log_dir=None, report_type=None, prob_threshold_fake=0.50,
    ...
    df['videoname'] = df['sample_name']
    df['frames'] = df['sample_name']
    df['norm_probability'] = df.apply(lambda x: norm_probability(x.predictions, x.probability), axis=1)
    all_videos = set(df['videoname'].values)

    final_df = pd.DataFrame(
        columns=['videoname', 'num_fake_frames', 'num_real_frames', 'total_number_frames', 'ground_truth'])
    ...
    for v in all_videos:
        ...
        final_df = final_df.append({'videoname': v, 'num_fake_frames': num_fake_frames,
    ...
    final_df = final_df.set_index(['videoname'])
