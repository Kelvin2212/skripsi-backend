...
def train_MRI_GAN_model(log_dir=None, train_resume_dir=None):
    ...
    csv_path = model_params['image_csv_path']
    image_folder = model_params['image_path']

    dataset = MRIDataset(csv_path, image_folder, transforms=data_transforms)

    test_size = int(len(dataset) * 0.2)
    train_dataset, test_dataset = torch.utils.data.random_split(dataset, [len(dataset) - test_size, test_size])

    # train_dataset = MRIDataset(csv_path, image_folder, transforms=data_transforms, frac=model_params['frac'], train=True)
    # test_dataset = MRIDataset(csv_path, image_folder, transforms=data_transforms, train=False)
