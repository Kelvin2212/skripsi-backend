...
def predict_deepfake(input_videofile, df_method, debug=True, verbose=False):
    ...
    crop_faces_from_video(input_videofile, output_path,
                          plain_faces_data_path, overwrite=True)
    ...
    if df_method == 'plain_frames':
        ...
        predict_mri_using_MRI_GAN(
            plain_faces_data_path, mri_output, vid, 256, overwrite=True)
        # model_path = 'assets/weights/deepfake_MRI_weights.chkpt'
        model_path = './DeepFakeDetectModel.chkpt'
        # model_path = '/content/drive/MyDrive/GAN/mri_gan_deepfake/logs/08-Mar-2021_07_53_22/highest_acc/DeepFakeDetectModel.chkpt'
        # model_path = '/content/drive/MyDrive/GAN/mri_gan_deepfake/logs/02-Nov-2023_19_54_32/highest_acc/DeepFakeDetectModel.chkpt'
    ...
    model = DeepFakeDetectModel(
        frame_dim=model_params['imsize'], encoder_name=model_params['encoder_name'])
    if verbose:
    ...
    model.eval()
    # torch.save(model, 'final_gan_model.pth')
    torch.save(model.state_dict(), './gan_model.pth')

    test_transform = torchvision.transforms.Compose([
        torchvision.transforms.Resize(
            (model_params['imsize'], model_params['imsize'])),
    ...
    test_dataset = SimpleImageFolder(
        root=data_path, transforms_=test_transform)
    ...
    with torch.no_grad():
        ...
        fake_frames_high_prob = probabilities[probabilities >=
                                              prob_threshold_fake]
        number_fake_frames = len(fake_frames_high_prob)
        if number_fake_frames == 0:
            fake_prob = 0
        else:
            fake_prob = round(sum(fake_frames_high_prob) /
                              number_fake_frames, 4)

        real_frames_high_prob = probabilities[probabilities <
                                              prob_threshold_fake]
        ...
        if number_real_frames == 0:
            ...
            real_prob = 1 - \
                round(sum(real_frames_high_prob) / number_real_frames, 4)
...
def get_gan(input_videofile, method="MRI"):
    ...
    fake_prob, real_prob, pred = predict_deepfake(
        input_videofile, method, debug=debug, verbose=verbose)
    if pred is None:
    ...
    print_line()
    if pred == 0:
        print_green(f'The Image is {label}, probability={probability}%')
    else:
        print_red(f'The Image is {label}, probability={probability}%')
    print_line()
...
def individual_test():
    ...
    debug = True
    verbose = True
    print(args.input_videofile)
    print(args.method)
    fake_prob, real_prob, pred = predict_deepfake(
        args.input_videofile, args.method, debug=debug, verbose=verbose)
    if pred is None:
        print_red('Failed to detect DeepFakes')
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_videofile', action='store',
