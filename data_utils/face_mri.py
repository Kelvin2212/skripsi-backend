...
def generate_MRI_dataset(test_size=0.2, dfdc_fract=0.5):
    ...
    real_train, real_test = train_test_split(dff, test_size=test_size)
    ...
    total_samples = dfr_base_len + dff_len

    print(f'Total Fake samples {dff_len}')
    print(f'Total Real samples {dfr_base_len}')
    ...
    print(
        f'Total samples {total_samples}, real={round(dfr_base_len / total_samples, 2)}% fake={round(dff_len / total_samples, 2)}%')
